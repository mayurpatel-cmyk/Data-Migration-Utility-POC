...
let DataValidationComponent = class DataValidationComponent {
    constructor() {
        this.toastr = inject(ToastrService);
        this.router = inject(Router);
        this.validationApi = inject(ValidationApiService);
        this.migrationService = inject(MigrationService);
        this.dataTransfer = inject(DataTransferService);
        ...
        this.showingValidPreview = {};
        this.selectedErrorJobIndex = -1;
        this.errorCurrentPage = 1;
        this.errorPageSize = 50;
    }
    toggleValidPreview(index) {
        this.showingValidPreview[index] = !this.showingValidPreview[index];
    }
    getPreviewRecords(records) {
    ...
    get paginatedErrorRecords() {
        if (this.selectedErrorJobIndex === -1 || !this.validationQueue[this.selectedErrorJobIndex])
            return [];
    ...
    get currentErrorMaxBound() {
        if (this.selectedErrorJobIndex === -1)
            return 0;
        const total = this.validationQueue[this.selectedErrorJobIndex].results.invalidRecords.length;
    ...
    getSfFieldMeta(fieldName) {
        if (!fieldName)
            return null;
        return this.sfFields.find((f) => f.name === fieldName);
    }
    getTotalRequiredFieldsCount() {
    ...
    async onFileSelected(event) {
        ...
        if ((file.name.endsWith('.xlsx') || file.name.endsWith('.xls')) && file.size > 50 * 1024 * 1024) {
            this.toastr.warning('Large Excel files process slowly. For best performance, consider converting this file to .CSV before uploading.', 'Large File Warning', { timeOut: 8000 });
        }
        const formData = new FormData();
        formData.append('file', file);
        try {
            this.toastr.info('Extracting file headers...', 'Reading File');
            const res = await firstValueFrom(this.validationApi.extractHeaders(formData));
    ...
    restoreSheetAndMappings(sheetName, previousMappings) {
    ...
    applyMassUpdate(job, csvField, value) {
        if (value === undefined)
            value = '';
        if (!job.results || !job.results.invalidRecords || job.results.invalidRecords.length === 0)
            return;
        let updatedCount = 0;
        job.results.invalidRecords.forEach((record) => {
    ...
    hasErrorsInColumn(job, csvField) {
        if (!job || !job.results || !job.results.invalidRecords)
            return false;
        const searchStr = `[${csvField}:`;
        return job.results.invalidRecords.some((record) => record.errors.includes(searchStr));
    }
    hasCellError(record, csvField) {
        if (!record || !record.errors)
            return false;
        const searchStr = `[${csvField}:`;
        return record.errors.includes(searchStr);
    }
    get allMappingsActive() {
        return this.mappings.length > 0 && this.mappings.every(m => m.isActive);
    }
    toggleMappingActive(mapping) {
        if (!mapping.isActive) {
            mapping.sfField = '';
            mapping.isDropdownOpen = false;
        }
    }
    toggleAllMappings(event) {
    ...
    updateFieldType(mapping, sfFieldName) {
        if (!sfFieldName) {
            mapping.type = 'string';
            return;
        }
    ...
    autoMapFields() {
        ...
        setTimeout(() => {
            let matchCount = 0;
            const normalizeString = (str) => String(str).toLowerCase().replace(/__c$/g, '').replace(/id$/g, '').replace(/[^a-z0-9]/g, '');
            this.mappings.forEach(mapping => {
                if (mapping.isActive && !mapping.sfField) {
                    ...
                    for (const field of this.sfFields) {
                        if (currentlyMappedSfFields.includes(field.name))
                            continue;
    ...
    addToQueue() {
        ...
        const cleanMappings = activeMappings.map(m => {
            ...
            const picklistVals = meta && meta.picklistValues
                ? meta.picklistValues.filter((p) => p.active).map((p) => p.value.toLowerCase())
                : [];
    ...
    async runValidationQueue() {
        ...
        for (const job of this.validationQueue) {
            ...
            try {
                const res = await firstValueFrom(this.validationApi.validateData(formData));
                job.results = res;
                job.status = 'done';
            }
            catch (error) {
        ...
        this.validationQueue.forEach(job => {
            if (job.results?.invalidRecords?.length > 0) {
                const duplicateRecords = job.results.invalidRecords
                    .filter((ir) => ir.errors.includes('Duplicate Record'))
                    .map((ir) => ({
    ...
    async revalidateJob(jobIndex) {
        const job = this.validationQueue[jobIndex];
        if (!job?.results?.invalidRecords?.length)
            return;
        this.isValidating = true;
        this.toastr.info(`Re-validating ${job.targetObject} records...`, 'Processing');
        this.cdr.detectChanges();
        const recordsToTest = job.results.invalidRecords.map((ir) => {
        ...
        try {
            const res = await firstValueFrom(this.validationApi.revalidateData(payload));
    ...
    downloadErrorLog() {
        ...
        this.validationQueue.forEach(job => {
            if (job.results?.invalidRecords?.length > 0) {
                const mappedErrors = job.results.invalidRecords.map((ir) => ({
                    SourceSheet: job.sheetName,
                    TargetObject: job.targetObject,
                    RowNumber: ir.rowNumber,
                    Errors: ir.errors,
    ...
    routeToMigration() {
        ...
        this.dataTransfer.setValidatedData(validJobs, `Cleaned_${this.selectedFile?.name || 'Batch.xlsx'}`, '');
            ...
            if (!this.parentObjectFieldsCache[parentObj]) {
                ...
                this.migrationService.getObjectFields(this.targetCrmId, parentObj, 'target').subscribe({
                    next: (response) => {
                        ...
                        this.parentObjectFieldsCache[parentObj] = fieldsArray.sort((a, b) => (a.label || '').localeCompare(b.label || ''));
    ...
    getParentFieldLabel(mapping, fieldName) {
        if (!fieldName)
            return '';
        // Dynamic label for Standard IDs based on CRM
        if (fieldName === 'Id')
            return `Id (Standard ${this.targetCrmId.toUpperCase()} ID)`;
        if (!mapping.parentObjectName)
            return fieldName;
        const parentFields = this.parentObjectFieldsCache[mapping.parentObjectName] || [];
        const field = parentFields.find((f) => f.name === fieldName);
        return field ? `${field.label} (${field.name})` : fieldName;
    }
    getFilteredParentFields(mapping) {
        if (!mapping.parentObjectName)
            return [];
        const parentFields = this.parentObjectFieldsCache[mapping.parentObjectName] || [];
        if (!mapping.parentSearchQuery)
            return parentFields;
...
], DataValidationComponent.prototype, "clickout", null);
DataValidationComponent = __decorate([
    Component({
...
export { DataValidationComponent };
//# sourceMappingURL=data-validation.component.js.map