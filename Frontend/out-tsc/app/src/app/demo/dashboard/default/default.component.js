...
import { Component, inject, ChangeDetectorRef, HostListener, ElementRef, signal } from '@angular/core';
...
let DefaultComponent = class DefaultComponent {
    constructor() {
        ...
        this.previewItemData = [];
        this.previewItemHeaders = [];
        ...
        this.upsertKeySearchQuery = '';
        this.displayName = signal('CRM User');
    ...
    ngOnInit() {
        ...
        if (transferred && transferred.data && Array.isArray(transferred.data) && transferred.data.length > 0) {
            ...
            transferred.data.forEach((job, index) => {
            ...
            this.migrationQueue = [];
            transferred.data.forEach((job, index) => {
                const enhancedMappings = job.mappings.map((m) => ({
                    csvField: m.csvField,
                    sfField: m.sfField,
                    type: m.type,
    ...
    selectUpsertKeyOption(fieldName) {
    ...
    get filteredUpsertKeys() {
        if (!this.upsertSearchQuery)
            return this.validUpsertKeys;
        const query = this.upsertSearchQuery.toLowerCase();
        return this.validUpsertKeys.filter(f => (f.label || '').toLowerCase().includes(query) ||
            (f.name || '').toLowerCase().includes(query));
    }
    getUpsertKeyLabel(fieldName) {
        if (!fieldName)
            return '';
        const field = this.validUpsertKeys.find(f => f.name === fieldName);
        return field ? `${field.label} (${field.name})` : fieldName;
    }
    get hasDeleteInBatch() {
    ...
    onSheetSelect(sheetName) {
        ...
        if (this.workbook) {
            ...
            if (json.length > 0) {
                this.csvHeaders = json[0].map((h) => (h ? String(h).trim() : '')).filter((h) => h.length > 0);
            }
            else {
                this.csvHeaders = [];
    ...
    getFilteredUpsertKeys() {
        if (!this.upsertKeySearchQuery)
    ...
    getFilteredParentFields(mapping) {
        ...
        return parentFields.filter((f) => f.label?.toLowerCase().includes(lowerQuery) || f.name?.toLowerCase().includes(lowerQuery));
    }
    ...
    selectTargetObject(objName, stepContext) {
    ...
    getMissingRequiredFields() {
        ...
        if (crm === 'hubspot') {
            ...
            if (objLower === 'leads' || objLower === 'contacts')
                addIfInSchema('Last_Name');
            if (objLower === 'accounts')
                addIfInSchema('Account_Name');
            if (objLower === 'deals')
                addIfInSchema('Deal_Name');
        }
    ...
    onSheetChangeInMapping(newSheet) {
        setTimeout(() => {
    ...
    onOperationModeChange() {
        ...
        if (this.operationMode === 'upsert' && !this.targetExtIdField) {
            ...
            if (crm === 'hubspot' && objLower === 'contacts') {
                ...
                if (hasExtId)
                    this.selectUpsertKey('external_id');
            }
            else {
    ...
    async loadMappingTemplate() {
        ...
        const objectTemplates = templates.filter((t) => t.data.targetObject === this.selectedObject);
        if (objectTemplates.length === 0) {
            this.toastr.info(`No saved templates found for ${this.selectedObject}.`, 'No Templates');
            return;
        }
        const options = {};
        objectTemplates.forEach((t) => { options[t.name] = t.name; });
        ...
        if (selectedName) {
            const t = objectTemplates.find((x) => x.name === selectedName).data;
    ...
    onObjectChangeInMapping(newObject) {
        if (!newObject)
            return;
        this.isLoadingFields = true;
        this.showPreview = false;
    ...
    fetchObjectFields(objectName, isEditMode = false) {
    ...
    getDynamicSequenceWarning() {
        if (this.operationMode !== 'upsert' || !this.selectedObject)
            return null;
...
], DefaultComponent.prototype, "clickout", null);
