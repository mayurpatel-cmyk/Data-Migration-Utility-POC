...
let ApiMappingComponent = class ApiMappingComponent {
    constructor() {
        ...
        this.globalLoadingText = 'Loading...';
        this.globalLoadingSubText = 'Please wait...';
        // CRM Identifiers from previous step
        this.sourceCrmId = '';
        this.targetCrmId = '';
        this.sourceSystem = 'Unknown';
        this.targetSystem = 'Unknown';
        ...
        this.mappedCount = 0;
        this.isStrictMapping = false;
        this.hideMappedFields = false;
        // Execution Variables
        this.jobStatus = 'Idle';
        this.logMessages = [];
        this.customQuery = '';
        ...
        this.recoverableSessions = [];
        this.isSessionsLoading = false;
        // --- ADD THIS TEMPLATE CONSTANT ---
        this.ZENDESK_CUSTOM_OBJECT_TEMPLATE = `/* Zendesk Custom Object Query Template 
  ...
  "filter": {
    "$and": [
      { "custom_object_fields.your_field_key": { "$eq": "Your Value" } }
        ...
        this.HUBSPOT_SEARCH_TEMPLATE = `/* HubSpot Search Filter 
 - Leave blank to fetch all records.
...
*/
{
  "filterGroups": [
    ...
    resumeSession(sessionId, crm, object) {
    ...
    get visibleMappings() {
        ...
        if (this.mappingSearchQuery) {
            ...
            filtered = filtered.filter(m => (m.sourceLabel && m.sourceLabel.toLowerCase().includes(query)) ||
                (m.sourceField && m.sourceField.toLowerCase().includes(query)));
        }
        return filtered;
    }
    changePreviewLimit(newLimit) {
    ...
    getFieldTypeBadge(type) {
        ...
        if (t.includes('string') || t.includes('text') || t.includes('email'))
            return 'bg-info text-dark';
        if (t.includes('number') || t.includes('int') || t.includes('double') || t.includes('currency'))
            return 'bg-success';
        if (t.includes('boolean'))
            return 'bg-warning text-dark';
        if (t.includes('reference') || t.includes('lookup'))
            return 'bg-primary';
        if (t.includes('date') || t.includes('time'))
            return 'bg-danger';
        if (t.includes('picklist'))
            return 'bg-secondary';
        return 'bg-secondary';
    }
    get hasPendingEdits() {
        if (!this.validationResults?.invalidRecords)
            return false;
        // Check if ANY record has the _editedFields object with at least one true value
        return this.validationResults.invalidRecords.some((rec) => rec._editedFields && Object.keys(rec._editedFields).length > 0);
    }
    isTypeMismatch(mapping) {
        ...
        if (srcType.includes('string') && ['string', 'text', 'textarea', 'picklist', 'reference'].includes(tgtType))
            return false;
    ...
    getIncompleteReferenceMappings() {
        const incomplete = [];
    ...
    getFilteredTargetFields(query, sourceFieldName) {
        ...
        if (query) {
            ...
            filtered = filtered.filter((f) => f.label.toLowerCase().includes(lowerQuery) ||
                f.name.toLowerCase().includes(lowerQuery));
        }
        return filtered;
    }
    getTargetFieldLabel(fieldName) {
        if (!fieldName)
            return '';
        const field = this.targetFields.find((f) => f.name === fieldName);
        return field ? `${field.label} (${field.name})` : fieldName;
    }
    toggleSourceDropdown(event) {
        event.stopPropagation();
    ...
    async applyFilter() {
        if (!this.customQuery || !this.selectedSourceObject)
            return;
    ...
    validateQuery() {
        ...
        const applySquiggle = (errorMsg, offendingText) => {
        ...
        // ==========================================
            ...
            const cleanJsonText = this.customQuery.replace(/\/\*[\s\S]*?\*\//g, '').trim();
            if (cleanJsonText === '')
                return true; // Blank is fine (fetches all)
            if (!cleanJsonText.startsWith('{')) {
        ...
        // ==========================================
            ...
            while ((match = sqlRegex.exec(this.customQuery)) !== null) {
                ...
                if (val.includes('*') || val.includes('%') || val.toLowerCase() === 'null')
                    continue;
        ...
        forkJoin({
            ...
            next: ({ sourceObjs, targetObjs }) => {
                ...
                if (this.sourceEntities.length > 0) {
                    const defaultSrc = this.sourceEntities.find((e) => e.name.toLowerCase().includes('account') || e.name.toLowerCase().includes('ticket') || e.name.toLowerCase().includes('contacts'));
                ...
                if (this.targetEntities.length > 0) {
                    const defaultTgt = this.targetEntities.find((e) => e.name.toLowerCase().includes('account') || e.name.toLowerCase().includes('user') || e.name.toLowerCase().includes('contacts'));
    ...
    loadMetadata() {
        ...
        forkJoin({
            ...
            next: ({ sourceData, targetData }) => {
                ...
                this.mappings = (sourceData.fields || []).map((field) => ({
    ...
    autoMap() {
        ...
        this.mappings.forEach((m) => {
            ...
            this.targetFields.forEach(t => {
                ...
                const isForgivingTypeMatch = (srcType.includes('string') && ['string', 'text', 'textarea', 'picklist', 'reference'].includes(tgtType)) ||
    ...
    async validateData(isRevalidation = false, fixedRecords = []) {
    ...
    applyMassUpdate(sourceField, value) {
        if (value === undefined)
            value = '';
        ...
        this.validationResults.invalidRecords.forEach((record) => {
    ...
    hasErrorsInColumn(sourceField) {
        if (!this.validationResults?.invalidRecords)
            return false;
        const searchStr = `[${sourceField}:`;
        return this.validationResults.invalidRecords.some((record) => record.errors.includes(searchStr));
    }
    hasCellError(record, sourceField) {
        if (!record || !record.errors)
            return false;
        const searchStr = `[${sourceField}:`;
        return record.errors.includes(searchStr);
    }
    markAsEdited(record, fieldName) {
        if (!record._editedFields)
            record._editedFields = {};
        record._editedFields[fieldName] = true;
    }
    async revalidatePreview() {
        if (!this.validationResults?.invalidRecords?.length)
            return;
        // Grab all the rows currently in the error grid (including the user's edits)
        const recordsToTest = this.validationResults.invalidRecords.map((ir) => ir.originalRow);
    ...
    runMigration() {
        if (this.hasPendingEdits) {
            ...
            const revalBtn = document.querySelector('.btn-danger.fw-bold');
            if (revalBtn) {
    ...
    executeMigrationJob(activeMappings) {
        this.successData = [];
        this.errorData = [];
        ...
        const fixedRecords = this.validationResults?.invalidRecords
            ?.filter((rec) => rec._editedFields)
            ?.map((rec) => rec.originalRow) || [];
        const job = {
            sessionId: this.currentSessionId,
            fixedRecords: fixedRecords,
...
], ApiMappingComponent.prototype, "clickout", null);
ApiMappingComponent = __decorate([
    Component({
...
export { ApiMappingComponent };
//# sourceMappingURL=API-mapping.component.js.map