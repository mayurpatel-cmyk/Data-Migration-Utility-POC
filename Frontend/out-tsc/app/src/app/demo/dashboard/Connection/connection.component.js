...
let ConnectionComponent = class ConnectionComponent {
    constructor() {
        ...
        this.crmAuthService = inject(CrmAuthService);
    ...
    ngOnInit() {
        ...
        this.authSubscription = this.route.queryParams.pipe(switchMap(params => {
    ...
    loadActiveConnections() {
        this.isPageLoading = true;
        this.crmAuthService.getUserConnections().pipe(delay(0)).subscribe({
            next: (connections) => {
                this.parseConnections(connections);
        ...
        connections.forEach(conn => {
            if (conn.connection_role === 'source') {
                ...
                if (conn.crm_type === 'salesforce') {
                    ...
                    if (conn.environment)
                        this.targetSalesforceEnv = conn.environment;
                }
                else if (conn.crm_type === 'zoho') {
                    nextTargetUrl = conn.api_domain || '';
                    if (conn.region)
                        this.targetZohoRegion = conn.region;
                }
                else if (conn.crm_type === 'zendesk') {
                    nextTargetUrl = conn.subdomain ? `https://${conn.subdomain}.zendesk.com` : '';
                    if (conn.subdomain)
                        this.targetZendeskSubdomain = conn.subdomain;
                }
            }
        });
