...
import { CommonModule, Location } from '@angular/common'; // 🟢 ADDED: Location Service context
import { Component, OnInit, inject, ChangeDetectorRef, NgZone, HostListener, OnDestroy } from '@angular/core';
import { FormsModule } from '@angular/forms';
import { Router } from '@angular/router';
import { forkJoin } from 'rxjs';
...
export class ApiMappingComponent implements OnInit, OnDestroy {
  ...
  private zone = inject(NgZone);
  private toastr = inject(ToastrService);
  private location = inject(Location); // 🟢 INJECTED: Native context back router tracking
  private validationSocket: WebSocket | null = null;
  private migrationSocket: WebSocket | null = null;
  ...
  // 🟢 ADDED: Dynamic window pop handling routing trigger back routine
  goBack(): void {
    this.location.back();
  }
      ...
      this.completionProvider = monaco.languages.registerCompletionItemProvider('sql', {
        provideCompletionItems: (model: any, position: any) => {
          ...
          if (crm === 'zendesk') {
            ...
            zdKeywords.forEach(kw => {
              suggestions.push({
                label: kw, kind: monaco.languages.CompletionItemKind.Keyword, insertText: kw, range: range
              });
            });
            ...
            if (this.sourceFields && this.sourceFields.length > 0) {
              this.sourceFields.forEach(field => {
                suggestions.push({
                  label: field.name, detail: `${field.label} (${field.type})`, kind: monaco.languages.CompletionItemKind.Field, insertText: field.name + ':', range: range
                });
              });
            }
          } else {
...
*/
{
  ...
  getFilteredTargetFields(query: string | undefined, sourceFieldName: string): any[] {
    ...
    if (query) {
      const lowerQuery = query.toLowerCase();
      filtered = filtered.filter((f) =>
        f.label.toLowerCase().includes(lowerQuery) || f.name.toLowerCase().includes(lowerQuery)
      );
    }
    return filtered;
  }
  ...
  getFilteredSourceEntities(): any[] {
    ...
    const lowerQuery = this.sourceSearchQuery.toLowerCase();
    return this.sourceEntities.filter(e =>
      e.label.toLowerCase().includes(lowerQuery) || e.name.toLowerCase().includes(lowerQuery)
    );
  }
  ...
  get queryContext() {
    ...
    if (crm === 'zendesk') {
      ...
      return {
        ...
        placeholder: isCustom ? '{\n  "filter": {\n    "$and": [\n      { "custom_object_fields.your_field": { "$eq": "value" } }\n    ]\n  }\n}' : "e.g., type:ticket status<solved created>2023-01-01",
        helpText: isCustom ? "Use JSON. Prefix custom fields with 'custom_object_fields.'. Leave blank for all records." : "Use Zendesk native search syntax to filter by tags, status, or dates.",
        icon: 'icon-search', buttonText: 'Apply Filter', loadingText: 'Filtering...'
      };
    } else if (crm === 'salesforce') {
      return {
        title: 'SOQL Query Editor', placeholder: "e.g., StageName = 'Closed Won' AND Amount > 5000",
        helpText: "Enter the WHERE clause for your Salesforce SOQL query (omit 'SELECT' and 'WHERE').",
        icon: 'icon-database', buttonText: 'Run Query', loadingText: 'Querying...'
      };
    } else if (crm === 'zoho') {
      return {
        title: 'Zoho COQL Filter', placeholder: "e.g., Account_Name != null and Industry = 'Technology'",
        helpText: "Enter Zoho criteria or a COQL condition to filter your records.",
        icon: 'icon-filter', buttonText: 'Run Query', loadingText: 'Querying...'
      };
    } else if (crm === 'hubspot') {
      return {
        title: 'HubSpot Filter & Search API', placeholder: "e.g., Suresh (Global Text Search) OR firstname=John (Property Match)",
        helpText: "Enter keywords or property definitions using key=value syntax to filter.",
        icon: 'icon-search', buttonText: 'Search Data', loadingText: 'Searching...'
      };
    } else {
      return {
        title: `${this.sourceSystem} Query Editor`, placeholder: "e.g., status = 'Active' OR created_at > '2024-01-01'",
        helpText: `Enter the specific database query to extract records from ${this.sourceSystem}.`,
        icon: 'icon-terminal', buttonText: 'Run Query', loadingText: 'Querying...'
      };
    }
  }
  ...
  scrollToBottom() {
    ...
    if (logContainer) logContainer.scrollTop = logContainer.scrollHeight;
    const terminalCard = document.getElementById('execution-terminal');
    if (terminalCard) terminalCard.scrollIntoView({ behavior: 'smooth', block: 'end' });
  }
  ...
  async applyFilter() {
    ...
    const payload = {
      crmId: this.sourceCrmId, objectName: this.selectedSourceObject, query: safeQuery,
      headers: this.previewHeaders, limit: this.previewLimit,
      sfToken: localStorage.getItem('sf_token') || '', sfInstance: localStorage.getItem('sf_instance_url') || '',
      zdToken: localStorage.getItem('zd_token') || '', zdSubdomain: localStorage.getItem('zd_subdomain') || '',
      zohoToken: localStorage.getItem('zoho_token') || '', zohoDomain: localStorage.getItem('zoho_api_domain') || '',
      hsToken: localStorage.getItem('hubspot_token') || '', hsRefresh: localStorage.getItem('hubspot_refresh_token') || ''
    };
  ...
  validateQuery(): boolean {
      ...
      if (this.monacoEditorInstance && offendingText) {
        ...
        if (startIndex !== -1) {
          ...
          monaco.editor.setModelMarkers(model, 'sql-validation', [{
            ...
            message: errorMsg, severity: monaco.MarkerSeverity.Error
          }]);
        }
      }
      return false;
    ...
    if (crm === 'zendesk') {
      ...
      if (hasSelect) {
        if (hasWhere) conditionPart = queryLower.split(' where ')[1];
      } else {
        conditionPart = queryLower;
      }
  ...
  autoMap() {
    ...
    this.mappings.forEach((m) => {
      ...
      this.targetFields.forEach(t => {
        ...
        else if (tgtLabelExact === srcLabelExact) score += 90;
        else if (tgtApiClean === srcApiClean) score += 80;
        else if (tgtLabelClean === srcLabelClean) score += 70;
        else if (srcApiClean.length > 3 && (tgtApiClean.includes(srcApiClean) || srcApiClean.includes(tgtApiClean))) score += 40;

        if (score >= 40 && isExactTypeMatch) score += 25;
        else if (score >= 40 && isForgivingTypeMatch) score += 10;
  ...
  async validateData(isRevalidation: boolean = false, fixedRecords: any[] = []) {
    if (this.mappedCount === 0) {
      this.toastr.error('Validation Aborted: Map fields to validate data.');
      return;
    }
    if (!isRevalidation && this.customQuery && !this.validateQuery()) {
      this.jobStatus = 'Validation Failed';
    ...
    if (!isRevalidation) {
      const confirmResult = await Swal.fire({
        title: 'Validate Entire Database?',
        text: `This will securely stream and test live records.`,
        icon: 'info', showCancelButton: true, confirmButtonColor: '#0d6efd', confirmButtonText: 'Yes, Start Validation'
      });
      if (!confirmResult.isConfirmed) return;
    ...
    const activeMappings = this.mappings
      .filter(m => m.targetField)
      .map(m => ({
        csvField: m.sourceField, sfField: m.targetField,
        type: this.targetFields.find(t => t.name === m.targetField)?.type || 'string'
      }));

    const sfRules: any = {};
    this.targetFields.forEach(field => { sfRules[field.name] = field; });
    ...
    const payload = {
      isRevalidation: isRevalidation, sessionId: this.currentSessionId, fixedRecords: fixedRecords,
      crmId: this.sourceCrmId, objectName: this.selectedSourceObject, query: safeQuery,
      mappings: activeMappings, dedupeKey: this.externalIdField, sfRules: sfRules,
      sfToken: localStorage.getItem('sf_token') || '', sfInstance: localStorage.getItem('sf_instance_url') || '',
      zdToken: localStorage.getItem('zd_token') || '', zdSubdomain: localStorage.getItem('zd_subdomain') || '',
      zohoToken: localStorage.getItem('zoho_token') || '', zohoDomain: localStorage.getItem('zoho_api_domain') || '',
      hsToken: localStorage.getItem('hubspot_token') || '', hsRefresh: localStorage.getItem('hubspot_refresh_token') || ''
    ...
    ws.onmessage = (event) => {
      this.zone.run(() => {
        ...
        if (data.stats) this.aggregateStats = data.stats;
        if ((data.status === 'Validation Passed' || data.status === 'Validation Warning') && data.invalidRecords) {
           this.validationResults.invalidRecords = data.invalidRecords;
  ...
  runMigration() {
    if (this.hasPendingEdits) {
      this.toastr.warning('Validate fixes before migrating.', 'Action Required');
      return;
    }
    if (this.customQuery && !this.validateQuery()) return;

    const activeMappings = this.mappings.filter(m => m.targetField !== '');
    if (activeMappings.length === 0) {
      this.toastr.warning('Map fields before migrating.', 'No Mappings');
      return;
    }
    ...
    if (missingFields.length > 0 || incompleteRefs.length > 0) {
      ...
      Swal.fire({
        title: 'Ready to Migrate!',
        text: `Execute ${this.operationMode.toUpperCase()} job?`,
        icon: 'info', showCancelButton: true, confirmButtonColor: '#198754', confirmButtonText: 'Yes, Run Job!'
      }).then((result) => {
        if (result.isConfirmed) this.executeMigrationJob(activeMappings);
      });
    }
  }
  ...
  private executeMigrationJob(activeMappings: any[]) {
    ...
    const enhancedMappings = activeMappings.map(m => {
      ...
      return {
        ...
        type: isRef ? 'reference' : fieldMeta?.type,
        referenceTo: fieldMeta?.referenceTo, relationshipName: fieldMeta?.relationshipName,
        relationalExtIdField: m.relationalExtIdField || (isRef ? 'Id' : undefined),
        parentObjectName: m.parentObjectName || (fieldMeta?.referenceTo ? fieldMeta.referenceTo[0] : undefined)
      };
    });
    ...
    const job = {
      sessionId: this.currentSessionId, fixedRecords: fixedRecords,
      sourceObject: this.selectedSourceObject, targetObject: this.selectedTargetObject,
      extractionQuery: safeQuery, mappings: enhancedMappings,
      operationMode: this.operationMode, batchSize: this.batchSize, externalIdField: this.externalIdField,
      sfToken: localStorage.getItem('sf_token') || '', sfInstance: localStorage.getItem('sf_instance_url') || '',
      zdToken: localStorage.getItem('zd_token') || '', zdSubdomain: localStorage.getItem('zd_subdomain') || '',
      zohoToken: localStorage.getItem('zoho_token') || '', zohoDomain: localStorage.getItem('zoho_api_domain') || '',
      hsToken: localStorage.getItem('hubspot_token') || '', hsRefresh: localStorage.getItem('hubspot_refresh_token') || ''
    ...
    ws.onmessage = (event) => {
      this.zone.run(() => {
        ...
        if (data.status) {
          this.jobStatus = data.status;
          if (data.status === 'Finished') {
            Swal.fire({ title: 'Migration Complete!', icon: 'success' });
          }
        }
        if (data.successData) this.successData = data.successData;
        if (data.errorData) this.errorData = data.errorData;
